...
/* USER CODE BEGIN PFP */
const volatile char* __attribute__((section(".ExtQSPIFlash"))) majStr = "HELLO";
/* USER CODE END PFP */
...
int main(void)
{
	...
	/*uint8_t szt = strlen((char*)majStr)+1;
	uint8_t* buff = (uint8_t*)malloc(szt);
	memmove(buff, (char*)majStr, szt);
	*(buff + szt) = 0x00;*/

	/*uint32_t *flashMem = (uint32_t *)0x90000000;
	uint32_t val = *flashMem;
	*flashMem = 0xffffffff;
	val = *flashMem;*/
